...
register_activation_hook( __FILE__, 'activate_theme_crud_plugin_function' );
register_deactivation_hook( __FILE__, 'deactivate_theme_crud_plugin_function' );
...
function activate_theme_crud_plugin_function() {
    global $wpdb;
    $charset_collate = $wpdb->get_charset_collate();
    $table_name = $wpdb->prefix . 'theme_crud';
  
    $sql = "CREATE TABLE $table_name (
      `id` bigint(11) unsigned NOT NULL AUTO_INCREMENT,
      `ThemeFormation` varchar(255),
      `description` text,
...
class EntryListTable extends WP_List_Table {
    ...
    function get_sortable_columns() {
      $sortable_columns = array(
        'ThemeFormation' => array('ThemeFormation', true)
      );
    ...
    function prepare_items() {
      ...
      $order = (isset($_REQUEST['order']) && in_array($_REQUEST['order'], array('asc', 'desc'))) ? $_REQUEST['order'] : 'desc';
        ...
        $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE `ThemeFormation` LIKE '%".$_REQUEST['s']."%' OR `description` LIKE '%".$_REQUEST['s']."%' ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged * $per_page), ARRAY_A);
		  } else {
